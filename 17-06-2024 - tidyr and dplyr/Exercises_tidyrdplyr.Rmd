...
3. calculate scaled variables with mean 0 and sd 1 for each variable (z-score) (hint: there is a faster way then manually defining each variable... with across()...)
    + extra: try once to mutate the table as a whole (columns are changed), and then alternatively, to add it to the end of the table to keep the original data in the same dataframe
...
2. calculate average length and sd of all columns
    + extra: change the name of columns into 'var.mean' instead of the standard 'var_mean'
...
#2. calculate average length and sd of all columns (again, think of across())
plants_tidy %>%
...
#1. Add the extra information to the plant dataset as a column
all_data <- plants_tidy %>% 
#+ replace NA's with zeros

#2. Make a dataset with plants that did not have fruits
no_fruits <- plants_tidy %>% 

#3. Make a dataset with only plants that have both types of measurements
inner_data <- plants_tidy %>% 
